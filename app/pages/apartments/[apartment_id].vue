    ...
    <div class="apartment__container">
      ...
      <div class="apartment__box">
        ...
        <div class="apartment__wrapper">
          ...
          />
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1724 1077" :key="sketch?.apartments">
            <NuxtLink
      ...
      <div class="apartment__main">
        ...
        <div class="apartment__content">
          <ul class="apartment__details">
            <li class="apartment__detail" v-for="(detail, i) in apartment?.details" :key="i">
              <span class="apartment__detail-rooms">
                {{ capitalize(detail[`key_${$i18n.locale}`]) }}
              </span>
              <h3 class="apartment__detail-area">{{ detail.val }} {{ $t('m-squared') }}</h3>
            </li>
          </ul>
        </div>
...
const makePDF = async () => {
  ...
  try {
    ...
    link.setAttribute('download', `${t('apartment').toLowerCase()}-${apartment.value?.unit}.pdf`);
