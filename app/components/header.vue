  ...
  <header class="header" :class="{ 'header--colorful': isColorful }">
    ...
    <div class="header__cta">
      <div class="header__locale">
        <button class="header__locale-content" @click="showLangDropdown = !showLangDropdown">
          <span>{{ capitalize($i18n.locale) }}</span>
          <IconsArrowDown />
        </button>
        <div class="header__locale-dropdown" :class="{ active: showLangDropdown }">
          <button
            v-for="(myLocale, index) in locales"
            :key="index"
            class="header__lang-button"
            :class="{ active: myLocale.code === locale }"
            @click="changeLocale(myLocale.code)"
          >
            {{ myLocale.name }}
          </button>
        </div>
...
const navigateSection = to => {
  if (route.name !== 'index') router.push({ path: localePath('/'), hash: to === '#' ? '' : to });
