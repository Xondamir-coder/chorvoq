      ...
      <div class="apartments__content">
        <ul class="apartments__details">
          <TransitionGroup name="fade">
            <li class="apartments__detail" v-for="(detail, i) in currentDetails" :key="i">
              <span class="apartments__detail-rooms">{{ detail.room }}</span>
              <h3 class="apartments__detail-area">{{ detail.area }} {{ $t('m-squared') }}</h3>
            </li>
          </TransitionGroup>
        </ul>
        <NuxtLink :to="$localePath('/genplan')" class="apartments__button">{{
          $t('learn-more')
        }}</NuxtLink>
      </div>
