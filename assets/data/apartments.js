import apartmentsSketchImg1 from '~/assets/images/apartments/1.webp';
import apartmentsSketchImg2 from '~/assets/images/apartments/2.webp';
import apartmentsSketchImg3 from '~/assets/images/apartments/3.webp';
import apartmentsSketchImg4 from '~/assets/images/apartments/4.webp';
import apartmentsSketchImg5 from '~/assets/images/apartments/5.webp';
import apartmentsSketchImg6 from '~/assets/images/apartments/6.webp';
import apartmentsSketchImg7 from '~/assets/images/apartments/7.webp';

import apartmentImg1 from '~/assets/images/apartment/1.png';
import apartmentImg2 from '~/assets/images/apartment/2.png';
import apartmentImg3 from '~/assets/images/apartment/3.png';
import apartmentImg4 from '~/assets/images/apartment/4.png';
import apartmentImg5 from '~/assets/images/apartment/5.png';
import apartmentImg6 from '~/assets/images/apartment/6.png';
import apartmentImg7 from '~/assets/images/apartment/7.png';
import apartmentImg8 from '~/assets/images/apartment/8.png';
import apartmentImg9 from '~/assets/images/apartment/9.png';
import apartmentImg10 from '~/assets/images/apartment/10.png';
import apartmentImg11 from '~/assets/images/apartment/11.png';
import apartmentImg12 from '~/assets/images/apartment/12.png';
import apartmentImg13 from '~/assets/images/apartment/13.png';
import apartmentImg14 from '~/assets/images/apartment/14.png';
import apartmentImg15 from '~/assets/images/apartment/15.png';
import apartmentImg16 from '~/assets/images/apartment/16.png';
import apartmentImg17 from '~/assets/images/apartment/17.png';
import apartmentImg18 from '~/assets/images/apartment/18.png';
import apartmentImg19 from '~/assets/images/apartment/19.png';
import apartmentImg20 from '~/assets/images/apartment/20.png';
import apartmentImg21 from '~/assets/images/apartment/21.png';
import apartmentImg22 from '~/assets/images/apartment/22.png';
import apartmentImg23 from '~/assets/images/apartment/23.png';
import apartmentImg24 from '~/assets/images/apartment/24.png';
import apartmentImg25 from '~/assets/images/apartment/25.png';
import apartmentImg26 from '~/assets/images/apartment/26.png';
import apartmentImg27 from '~/assets/images/apartment/27.png';
import apartmentImg28 from '~/assets/images/apartment/28.png';
import apartmentImg29 from '~/assets/images/apartment/29.png';
import apartmentImg30 from '~/assets/images/apartment/30.png';
import apartmentImg31 from '~/assets/images/apartment/31.png';
import apartmentImg32 from '~/assets/images/apartment/32.png';
import apartmentImg33 from '~/assets/images/apartment/33.png';
import apartmentImg34 from '~/assets/images/apartment/34.png';
import apartmentImg35 from '~/assets/images/apartment/35.png';
import apartmentImg36 from '~/assets/images/apartment/36.png';
import apartmentImg37 from '~/assets/images/apartment/37.png';
import apartmentImg38 from '~/assets/images/apartment/38.png';
import apartmentImg39 from '~/assets/images/apartment/39.png';
import apartmentImg40 from '~/assets/images/apartment/40.png';
import apartmentImg41 from '~/assets/images/apartment/41.png';
import apartmentImg42 from '~/assets/images/apartment/42.png';
import apartmentImg43 from '~/assets/images/apartment/43.png';
import apartmentImg44 from '~/assets/images/apartment/44.png';
import apartmentImg45 from '~/assets/images/apartment/45.png';
import apartmentImg46 from '~/assets/images/apartment/46.png';
import apartmentImg47 from '~/assets/images/apartment/47.png';
import apartmentImg48 from '~/assets/images/apartment/48.png';

// {apartment_id} detailed apartment data (E1)
export const apartments = [
	// 1 - Floor
	{
		phaseId: '1',
		apartmentId: '1',
		blockId: 'E1',
		floorId: '1',
		img: apartmentImg1,
		details: [
			{
				key_ru: 'Холл',
				key_en: 'Hall',
				key_uz: 'Dahliz',
				val: '7,00'
			},
			{
				key_ru: 'Кухня + Зал',
				key_en: 'Kitchen + Living room',
				key_uz: 'Oshxona + Mehmonxona',
				val: '22,61'
			},
			{
				key_ru: 'Детская',
				key_en: 'Childrens room',
				key_uz: 'Bolalar xonasi',
				val: '9,70'
			},
			{
				key_ru: 'Спальня',
				key_en: 'Bedroom',
				key_uz: 'Yotoqxona',
				val: '13,39'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,98'
			},
			{
				key_ru: 'Прач.',
				key_en: 'Loundry',
				key_uz: 'Kir yuvish xonasi',
				val: '1,05'
			},
			{
				key_ru: 'Терасса',
				key_en: 'Terrace',
				key_uz: 'Terassa',
				val: '18,03'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '2',
		blockId: 'E1',
		floorId: '1',
		img: apartmentImg2,
		details: [
			{
				key_ru: 'Холл',
				key_en: 'Hall',
				key_uz: 'Dahliz',
				val: '3,09'
			},
			{
				key_ru: 'Кухня + Зал',
				key_en: 'Kitchen + Living room',
				key_uz: 'Oshxona + Mehmonxona',
				val: '17,14'
			},
			{
				key_ru: 'Спальня',
				key_en: 'Bedroom',
				key_uz: 'Yotoqxona',
				val: '10,93'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,86'
			},
			{
				key_ru: 'Прач',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '0,76'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '3',
		blockId: 'E1',
		floorId: '1',
		img: apartmentImg3,
		details: [
			{
				key_ru: 'Холл',
				key_en: 'Hall',
				key_uz: 'Dahliz',
				val: '5,95'
			},
			{
				key_ru: 'Кухня + Зал',
				key_en: 'Kitchen + Living room',
				key_uz: 'Oshxona + Mehmonxona',
				val: '23,53'
			},
			{
				key_ru: 'Спальня',
				key_en: 'Bedroom',
				key_uz: 'Yotoqxona',
				val: '14,73'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '4,05'
			},
			{
				key_ru: 'Прач',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '1,00'
			},
			{
				key_ru: 'Терасса',
				key_en: 'Terrace',
				key_uz: 'Terassa',
				val: '20,73'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '4',
		blockId: 'E1',
		floorId: '1',
		img: apartmentImg4,
		details: [
			{
				key_ru: 'Холл',
				key_en: 'Hall',
				key_uz: 'Dahliz',
				val: '5,62'
			},
			{
				key_ru: 'Кухня + Зал',
				key_en: 'Kitchen + Living room',
				key_uz: 'Oshxona + Mehmonxona',
				val: '22,92'
			},
			{
				key_ru: 'Спальня',
				key_en: 'Bedroom',
				key_uz: 'Yotoqxona',
				val: '15,27'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,85'
			},
			{
				key_ru: 'Прач',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '1,21'
			},
			{
				key_ru: 'Терасса',
				key_en: 'Terrace',
				key_uz: 'Terassa',
				val: '21,17'
			}
		]
	},

	// 2 - Floor
	{
		phaseId: '1',
		apartmentId: '5',
		blockId: 'E1',
		floorId: '2',
		img: apartmentImg5,
		details: [
			{
				key_ru: 'Холл',
				key_en: 'Hall',
				key_uz: 'Dahliz',
				val: '2,05'
			},
			{
				key_ru: 'Кухня + Зал',
				key_en: 'Kitchen + Living room',
				key_uz: 'Oshxona + Mehmonxona',
				val: '23,48'
			},
			{
				key_ru: 'Спальня',
				key_en: 'Bedroom',
				key_uz: 'Yotoqxona',
				val: '11,77'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,72'
			},
			{
				key_ru: 'Прач',
				key_en: 'Laundry',
				key_uz: 'Kir yuvish xonasi',
				val: '1,13'
			},
			{
				key_ru: 'Балкон',
				key_en: 'Balcony',
				key_uz: 'Balkon',
				val: '4,40'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '6',
		blockId: 'E1',
		floorId: '2',
		img: apartmentImg6,
		details: [
			{
				key_ru: 'Холл',
				key_en: 'Hall',
				key_uz: 'Dahliz',
				val: '2,09'
			},
			{
				key_ru: 'Кухня + Зал',
				key_en: 'Kitchen + Living room',
				key_uz: 'Oshxona + Mehmonxona',
				val: '18,51'
			},
			{
				key_ru: 'Спальня',
				key_en: 'Bedroom',
				key_uz: 'Yotoqxona',
				val: '10,77'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,88'
			},
			{
				key_ru: 'Прач',
				key_en: 'Laundry',
				key_uz: 'Kir yuvish xonasi',
				val: '0,74'
			},
			{
				key_ru: 'Балкон',
				key_en: 'Balcony',
				key_uz: 'Balkon',
				val: '4,40'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '7',
		blockId: 'E1',
		floorId: '2',
		img: apartmentImg7,
		details: [
			{
				key_ru: 'Холл',
				key_en: 'Hall',
				key_uz: 'Dahliz',
				val: '3,09'
			},
			{
				key_ru: 'Кухня + Зал',
				key_en: 'Kitchen + Living room',
				key_uz: 'Oshxona + Mehmonxona',
				val: '17,14'
			},
			{
				key_ru: 'Спальня',
				key_en: 'Bedroom',
				key_uz: 'Yotoqxona',
				val: '10,93'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,86'
			},
			{
				key_ru: 'Прач',
				key_en: 'Loundry',
				key_uz: 'Kir yuvish xonasi',
				val: '0,76'
			},
			{
				key_ru: 'Балкон',
				key_en: 'Balcony',
				key_uz: 'Balkon',
				val: '4,42'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '8',
		blockId: 'E1',
		floorId: '2',
		img: apartmentImg8,
		details: [
			{
				key_ru: 'Жилая площадь',
				key_en: 'Living room',
				key_uz: 'Mehmonxona',
				val: '19,21'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,61'
			},
			{
				key_ru: 'Прач',
				key_en: 'Loundry',
				key_uz: 'Kir yuvish xonasi',
				val: '0,95'
			},
			{
				key_ru: 'Балкон',
				key_en: 'Balcony',
				key_uz: 'Balkon',
				val: '3,92'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '9',
		blockId: 'E1',
		floorId: '2',
		img: apartmentImg9,
		details: [
			{
				key_ru: 'Жилая площадь',
				key_en: 'Living room',
				key_uz: 'Mehmonxona',
				val: '19,78'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,56'
			},
			{
				key_ru: 'Прач',
				key_en: 'Loundry',
				key_uz: 'Kir yuvish xonasi',
				val: '0,86'
			},
			{
				key_ru: 'Балкон',
				key_en: 'Balcony',
				key_uz: 'Balkon',
				val: '3,48'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '10',
		blockId: 'E1',
		floorId: '2',
		img: apartmentImg10,
		details: [
			{
				key_ru: 'Жилая площадь',
				key_en: 'Living room',
				key_uz: 'Mehmonxona',
				val: '18,19'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,46'
			},
			{
				key_ru: 'Прач',
				key_en: 'Loundry',
				key_uz: 'Kir yuvish xonasi',
				val: '0,86'
			},
			{
				key_ru: 'Балкон',
				key_en: 'Balcony',
				key_uz: 'Balkon',
				val: '3,48'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '11',
		blockId: 'E1',
		floorId: '2',
		img: apartmentImg11,
		details: [
			{
				key_ru: 'Жилая площадь',
				key_en: 'Living room',
				key_uz: 'Mehmonxona',
				val: '25,79'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '5,62'
			},
			{
				key_ru: 'Прач',
				key_en: 'Loundry',
				key_uz: 'Kir yuvish xonasi',
				val: '1,82'
			},
			{
				key_ru: 'Балкон',
				key_en: 'Balcony',
				key_uz: 'Balkon',
				val: '3,67'
			}
		]
	},

	// 3 - Floor
	{
		phaseId: '1',
		apartmentId: '12',
		blockId: 'E1',
		floorId: '3',
		img: apartmentImg12,
		details: [
			{
				key_ru: 'Холл',
				key_en: 'Hall',
				key_uz: 'Dahliz',
				val: '2,05'
			},
			{
				key_ru: 'Кухня + Зал',
				key_en: 'Kitchen + Living room',
				key_uz: 'Oshxona + Mehmonxona',
				val: '23,48'
			},
			{
				key_ru: 'Спальня',
				key_en: 'Bedroom',
				key_uz: 'Yotoqxona',
				val: '11,77'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,72'
			},
			{
				key_ru: 'Прач',
				key_en: 'Laundry',
				key_uz: 'Kir yuvish xonasi',
				val: '1,13'
			},
			{
				key_ru: 'Балкон',
				key_en: 'Balcony',
				key_uz: 'Balkon',
				val: '4,40'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '13',
		blockId: 'E1',
		floorId: '3',
		img: apartmentImg13,
		details: [
			{
				key_ru: 'Холл',
				key_en: 'Hall',
				key_uz: 'Dahliz',
				val: '2,09'
			},
			{
				key_ru: 'Кухня + Зал',
				key_en: 'Kitchen + Living room',
				key_uz: 'Oshxona + Mehmonxona',
				val: '18,51'
			},
			{
				key_ru: 'Спальня',
				key_en: 'Bedroom',
				key_uz: 'Yotoqxona',
				val: '10,77'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,88'
			},
			{
				key_ru: 'Прач',
				key_en: 'Laundry',
				key_uz: 'Kir yuvish xonasi',
				val: '0,74'
			},
			{
				key_ru: 'Балкон',
				key_en: 'Balcony',
				key_uz: 'Balkon',
				val: '4,40'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '14',
		blockId: 'E1',
		floorId: '3',
		img: apartmentImg14,
		details: [
			{
				key_ru: 'Холл',
				key_en: 'Hall',
				key_uz: 'Dahliz',
				val: '3,09'
			},
			{
				key_ru: 'Кухня + Зал',
				key_en: 'Kitchen + Living room',
				key_uz: 'Oshxona + Mehmonxona',
				val: '17,14'
			},
			{
				key_ru: 'Спальня',
				key_en: 'Bedroom',
				key_uz: 'Yotoqxona',
				val: '10,93'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,86'
			},
			{
				key_ru: 'Прач',
				key_en: 'Laundry',
				key_uz: 'Kir yuvish xonasi',
				val: '0,76'
			},
			{
				key_ru: 'Балкон',
				key_en: 'Balcony',
				key_uz: 'Balkon',
				val: '4,42'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '15',
		blockId: 'E1',
		floorId: '3',
		img: apartmentImg15,
		details: [
			{
				key_ru: 'Жилая площадь',
				key_en: 'Living room',
				key_uz: 'Mehmonxona',
				val: '19,21'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,61'
			},
			{
				key_ru: 'Прач',
				key_en: 'Loundry',
				key_uz: 'Kir yuvish xonasi',
				val: '0,95'
			},
			{
				key_ru: 'Балкон',
				key_en: 'Balcony',
				key_uz: 'Balkon',
				val: '3,92'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '16',
		blockId: 'E1',
		floorId: '3',
		img: apartmentImg16,
		details: [
			{
				key_ru: 'Жилая площадь',
				key_en: 'Living room',
				key_uz: 'Mehmonxona',
				val: '19,78'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,56'
			},
			{
				key_ru: 'Прач',
				key_en: 'Loundry',
				key_uz: 'Kir yuvish xonasi',
				val: '0,86'
			},
			{
				key_ru: 'Балкон',
				key_en: 'Balcony',
				key_uz: 'Balkon',
				val: '3,48'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '17',
		blockId: 'E1',
		floorId: '3',
		img: apartmentImg17,
		details: [
			{
				key_ru: 'Жилая площадь',
				key_en: 'Living room',
				key_uz: 'Mehmonxona',
				val: '18,19'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,46'
			},
			{
				key_ru: 'Прач',
				key_en: 'Loundry',
				key_uz: 'Kir yuvish xonasi',
				val: '0,86'
			},
			{
				key_ru: 'Балкон',
				key_en: 'Balcony',
				key_uz: 'Balkon',
				val: '3,48'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '18',
		blockId: 'E1',
		floorId: '3',
		img: apartmentImg18,
		details: [
			{
				key_ru: 'Жилая площадь',
				key_en: 'Living room',
				key_uz: 'Mehmonxona',
				val: '25,79'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '5,62'
			},
			{
				key_ru: 'Прач',
				key_en: 'Loundry',
				key_uz: 'Kir yuvish xonasi',
				val: '1,82'
			},
			{
				key_ru: 'Балкон',
				key_en: 'Balcony',
				key_uz: 'Balkon',
				val: '3,67'
			}
		]
	},

	// 4 - Floor
	{
		phaseId: '1',
		apartmentId: '19',
		blockId: 'E1',
		floorId: '4',
		img: apartmentImg19,
		details: [
			{
				key_ru: 'Холл',
				key_en: 'Hall',
				key_uz: 'Dahliz',
				val: '2,05'
			},
			{
				key_ru: 'Кухня + Зал',
				key_en: 'Kitchen + Living room',
				key_uz: 'Oshxona + Mehmonxona',
				val: '23,48'
			},
			{
				key_ru: 'Спальня',
				key_en: 'Bedroom',
				key_uz: 'Yotoqxona',
				val: '11,77'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,72'
			},
			{
				key_ru: 'Прач',
				key_en: 'Laundry',
				key_uz: 'Kir yuvish xonasi',
				val: '1,13'
			},
			{
				key_ru: 'Балкон',
				key_en: 'Balcony',
				key_uz: 'Balkon',
				val: '4,40'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '20',
		blockId: 'E1',
		floorId: '4',
		img: apartmentImg20,
		details: [
			{
				key_ru: 'Холл',
				key_en: 'Hall',
				key_uz: 'Dahliz',
				val: '2,09'
			},
			{
				key_ru: 'Кухня + Зал',
				key_en: 'Kitchen + Living room',
				key_uz: 'Oshxona + Mehmonxona',
				val: '18,51'
			},
			{
				key_ru: 'Спальня',
				key_en: 'Bedroom',
				key_uz: 'Yotoqxona',
				val: '10,77'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,88'
			},
			{
				key_ru: 'Прач',
				key_en: 'Laundry',
				key_uz: 'Kir yuvish xonasi',
				val: '0,74'
			},
			{
				key_ru: 'Балкон',
				key_en: 'Balcony',
				key_uz: 'Balkon',
				val: '4,40'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '21',
		blockId: 'E1',
		floorId: '4',
		img: apartmentImg21,
		details: [
			{
				key_ru: 'Холл',
				key_en: 'Hall',
				key_uz: 'Dahliz',
				val: '3,09'
			},
			{
				key_ru: 'Кухня + Зал',
				key_en: 'Kitchen + Living room',
				key_uz: 'Oshxona + Mehmonxona',
				val: '17,14'
			},
			{
				key_ru: 'Спальня',
				key_en: 'Bedroom',
				key_uz: 'Yotoqxona',
				val: '10,93'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,86'
			},
			{
				key_ru: 'Прач',
				key_en: 'Laundry',
				key_uz: 'Kir yuvish xonasi',
				val: '0,76'
			},
			{
				key_ru: 'Балкон',
				key_en: 'Balcony',
				key_uz: 'Balkon',
				val: '4,42'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '22',
		blockId: 'E1',
		floorId: '4',
		img: apartmentImg22,
		details: [
			{
				key_ru: 'Жилая площадь',
				key_en: 'Living room',
				key_uz: 'Mehmonxona',
				val: '19,21'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,61'
			},
			{
				key_ru: 'Прач',
				key_en: 'Loundry',
				key_uz: 'Kir yuvish xonasi',
				val: '0,95'
			},
			{
				key_ru: 'Балкон',
				key_en: 'Balcony',
				key_uz: 'Balkon',
				val: '3,92'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '23',
		blockId: 'E1',
		floorId: '4',
		img: apartmentImg23,
		details: [
			{
				key_ru: 'Жилая площадь',
				key_en: 'Living room',
				key_uz: 'Mehmonxona',
				val: '19,78'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,56'
			},
			{
				key_ru: 'Прач',
				key_en: 'Loundry',
				key_uz: 'Kir yuvish xonasi',
				val: '0,86'
			},
			{
				key_ru: 'Балкон',
				key_en: 'Balcony',
				key_uz: 'Balkon',
				val: '3,48'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '24',
		blockId: 'E1',
		floorId: '4',
		img: apartmentImg24,
		details: [
			{
				key_ru: 'Жилая площадь',
				key_en: 'Living room',
				key_uz: 'Mehmonxona',
				val: '18,19'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,46'
			},
			{
				key_ru: 'Прач',
				key_en: 'Loundry',
				key_uz: 'Kir yuvish xonasi',
				val: '0,86'
			},
			{
				key_ru: 'Балкон',
				key_en: 'Balcony',
				key_uz: 'Balkon',
				val: '3,48'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '25',
		blockId: 'E1',
		floorId: '4',
		img: apartmentImg25,
		details: [
			{
				key_ru: 'Жилая площадь',
				key_en: 'Living room',
				key_uz: 'Mehmonxona',
				val: '19,98'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,83'
			},
			{
				key_ru: 'Прач',
				key_en: 'Loundry',
				key_uz: 'Kir yuvish xonasi',
				val: '0,86'
			},
			{
				key_ru: 'Балкон',
				key_en: 'Balcony',
				key_uz: 'Balkon',
				val: '3,83'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '26',
		blockId: 'E1',
		floorId: '4',
		img: apartmentImg26,
		details: [
			{
				key_ru: 'Кухня + Зал',
				key_en: 'Kitchen + Living room',
				key_uz: 'Oshxona + Mehmonxona',
				val: '22,31'
			},
			{
				key_ru: 'Холл',
				key_en: 'Hall',
				key_uz: 'Dahliz',
				val: '2,17'
			},
			{
				key_ru: 'Спальня',
				key_en: 'Bedroom',
				key_uz: 'Yotoqxona',
				val: '10,67'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,72'
			},
			{
				key_ru: 'Терасса',
				key_en: 'Terrace',
				key_uz: 'Terassa',
				val: '49,61'
			},
			{
				key_ru: 'Прач',
				key_en: 'Loundry',
				key_uz: 'Kir yuvish xonasi',
				val: '0,87'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '27',
		blockId: 'E1',
		floorId: '4',
		img: apartmentImg27,
		details: [
			{
				key_ru: 'Кухня + Зал',
				key_en: 'Kitchen + Living room',
				key_uz: 'Oshxona + Mehmonxona',
				val: '26,31'
			},
			{
				key_ru: 'Холл',
				key_en: 'Hall',
				key_uz: 'Dahliz',
				val: '3,22'
			},
			{
				key_ru: 'Спальня',
				key_en: 'Bedroom',
				key_uz: 'Yotoqxona',
				val: '16,74'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '6,72'
			},
			{
				key_ru: 'Терасса',
				key_en: 'Terrace',
				key_uz: 'Terassa',
				val: '44,88'
			},
			{
				key_ru: 'Прач',
				key_en: 'Loundry',
				key_uz: 'Kir yuvish xonasi',
				val: '0,54'
			}
		]
	},

	// 5 - Floor
	{
		phaseId: '1',
		apartmentId: '28',
		blockId: 'E1',
		floorId: '5',
		img: apartmentImg28,
		details: [
			{
				key_ru: 'Холл',
				key_en: 'Hall',
				key_uz: 'Dahliz',
				val: '2,05'
			},
			{
				key_ru: 'Кухня + Зал',
				key_en: 'Kitchen + Living room',
				key_uz: 'Oshxona + Mehmonxona',
				val: '23,48'
			},
			{
				key_ru: 'Спальня',
				key_en: 'Bedroom',
				key_uz: 'Yotoqxona',
				val: '11,77'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,72'
			},
			{
				key_ru: 'Прач',
				key_en: 'Laundry',
				key_uz: 'Kir yuvish xonasi',
				val: '1,13'
			},
			{
				key_ru: 'Балкон',
				key_en: 'Balcony',
				key_uz: 'Balkon',
				val: '4,40'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '29',
		blockId: 'E1',
		floorId: '5',
		img: apartmentImg29,
		details: [
			{
				key_ru: 'Холл',
				key_en: 'Hall',
				key_uz: 'Dahliz',
				val: '2,09'
			},
			{
				key_ru: 'Кухня + Зал',
				key_en: 'Kitchen + Living room',
				key_uz: 'Oshxona + Mehmonxona',
				val: '18,51'
			},
			{
				key_ru: 'Спальня',
				key_en: 'Bedroom',
				key_uz: 'Yotoqxona',
				val: '10,77'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,88'
			},
			{
				key_ru: 'Прач',
				key_en: 'Laundry',
				key_uz: 'Kir yuvish xonasi',
				val: '0,74'
			},
			{
				key_ru: 'Балкон',
				key_en: 'Balcony',
				key_uz: 'Balkon',
				val: '4,40'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '30',
		blockId: 'E1',
		floorId: '5',
		img: apartmentImg30,
		details: [
			{
				key_ru: 'Холл',
				key_en: 'Hall',
				key_uz: 'Dahliz',
				val: '3,09'
			},
			{
				key_ru: 'Кухня + Зал',
				key_en: 'Kitchen + Living room',
				key_uz: 'Oshxona + Mehmonxona',
				val: '17,14'
			},
			{
				key_ru: 'Спальня',
				key_en: 'Bedroom',
				key_uz: 'Yotoqxona',
				val: '10,93'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,86'
			},
			{
				key_ru: 'Прач',
				key_en: 'Laundry',
				key_uz: 'Kir yuvish xonasi',
				val: '0,76'
			},
			{
				key_ru: 'Балкон',
				key_en: 'Balcony',
				key_uz: 'Balkon',
				val: '4,42'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '31',
		blockId: 'E1',
		floorId: '5',
		img: apartmentImg31,
		details: [
			{
				key_ru: 'Жилая площадь',
				key_en: 'Living room',
				key_uz: 'Mehmonxona',
				val: '19,21'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,61'
			},
			{
				key_ru: 'Прач',
				key_en: 'Loundry',
				key_uz: 'Kir yuvish xonasi',
				val: '0,95'
			},
			{
				key_ru: 'Балкон',
				key_en: 'Balcony',
				key_uz: 'Balkon',
				val: '3,92'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '32',
		blockId: 'E1',
		floorId: '5',
		img: apartmentImg32,
		details: [
			{
				key_ru: 'Жилая площадь',
				key_en: 'Living room',
				key_uz: 'Mehmonxona',
				val: '19,78'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,56'
			},
			{
				key_ru: 'Прач',
				key_en: 'Loundry',
				key_uz: 'Kir yuvish xonasi',
				val: '0,86'
			},
			{
				key_ru: 'Балкон',
				key_en: 'Balcony',
				key_uz: 'Balkon',
				val: '3,48'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '33',
		blockId: 'E1',
		floorId: '5',
		img: apartmentImg33,
		details: [
			{
				key_ru: 'Жилая площадь',
				key_en: 'Living room',
				key_uz: 'Mehmonxona',
				val: '18,19'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,46'
			},
			{
				key_ru: 'Прач',
				key_en: 'Loundry',
				key_uz: 'Kir yuvish xonasi',
				val: '0,86'
			},
			{
				key_ru: 'Балкон',
				key_en: 'Balcony',
				key_uz: 'Balkon',
				val: '3,48'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '34',
		blockId: 'E1',
		floorId: '5',
		img: apartmentImg34,
		details: [
			{
				key_ru: 'Жилая площадь',
				key_en: 'Living room',
				key_uz: 'Mehmonxona',
				val: '19,98'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,83'
			},
			{
				key_ru: 'Прач',
				key_en: 'Loundry',
				key_uz: 'Kir yuvish xonasi',
				val: '0,86'
			},
			{
				key_ru: 'Балкон',
				key_en: 'Balcony',
				key_uz: 'Balkon',
				val: '3,71'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '35',
		blockId: 'E1',
		floorId: '5',
		img: apartmentImg35,
		details: [
			{
				key_ru: 'Холл',
				key_en: 'Hall',
				key_uz: 'Dahliz',
				val: '2,17'
			},
			{
				key_ru: 'Кухня + Зал',
				key_en: 'Kitchen + Living room',
				key_uz: 'Oshxona + Mehmonxona',
				val: '22,31'
			},
			{
				key_ru: 'Спальня',
				key_en: 'Bedroom',
				key_uz: 'Yotoqxona',
				val: '10,76'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '6,72'
			},
			{
				key_ru: 'Прач.',
				key_en: 'Loundry',
				key_uz: 'Kir yuvish xonasi',
				val: '0,87'
			},
			{
				key_ru: 'Балкон',
				key_en: 'Balcony',
				key_uz: 'Balkon',
				val: '9,86'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '36',
		blockId: 'E1',
		floorId: '5',
		img: apartmentImg36,
		details: [
			{
				key_ru: 'Холл',
				key_en: 'Hall',
				key_uz: 'Dahliz',
				val: '3,22'
			},
			{
				key_ru: 'Кухня + Зал',
				key_en: 'Kitchen + Living room',
				key_uz: 'Oshxona + Mehmonxona',
				val: '26,31'
			},
			{
				key_ru: 'Спальня',
				key_en: 'Bedroom',
				key_uz: 'Yotoqxona',
				val: '16,74'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '6,72'
			},
			{
				key_ru: 'Прач.',
				key_en: 'Loundry',
				key_uz: 'Kir yuvish xonasi',
				val: '0,54'
			}
		]
	},

	// 6 - Floor
	{
		phaseId: '1',
		apartmentId: '37',
		blockId: 'E1',
		floorId: '6',
		img: apartmentImg37,
		details: [
			{
				key_ru: 'Холл',
				key_en: 'Hall',
				key_uz: 'Dahliz',
				val: '2,05'
			},
			{
				key_ru: 'Кухня + Зал',
				key_en: 'Kitchen + Living room',
				key_uz: 'Oshxona + Mehmonxona',
				val: '23,48'
			},
			{
				key_ru: 'Спальня',
				key_en: 'Bedroom',
				key_uz: 'Yotoqxona',
				val: '11,77'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,72'
			},
			{
				key_ru: 'Прач',
				key_en: 'Laundry',
				key_uz: 'Kir yuvish xonasi',
				val: '1,13'
			},
			{
				key_ru: 'Балкон',
				key_en: 'Balcony',
				key_uz: 'Balkon',
				val: '4,40'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '38',
		blockId: 'E1',
		floorId: '6',
		img: apartmentImg38,
		details: [
			{
				key_ru: 'Холл',
				key_en: 'Hall',
				key_uz: 'Dahliz',
				val: '2,09'
			},
			{
				key_ru: 'Кухня + Зал',
				key_en: 'Kitchen + Living room',
				key_uz: 'Oshxona + Mehmonxona',
				val: '18,51'
			},
			{
				key_ru: 'Спальня',
				key_en: 'Bedroom',
				key_uz: 'Yotoqxona',
				val: '10,77'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,88'
			},
			{
				key_ru: 'Прач',
				key_en: 'Laundry',
				key_uz: 'Kir yuvish xonasi',
				val: '0,74'
			},
			{
				key_ru: 'Балкон',
				key_en: 'Balcony',
				key_uz: 'Balkon',
				val: '4,40'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '39',
		blockId: 'E1',
		floorId: '6',
		img: apartmentImg39,
		details: [
			{
				key_ru: 'Холл',
				key_en: 'Hall',
				key_uz: 'Dahliz',
				val: '3,09'
			},
			{
				key_ru: 'Кухня + Зал',
				key_en: 'Kitchen + Living room',
				key_uz: 'Oshxona + Mehmonxona',
				val: '17,14'
			},
			{
				key_ru: 'Спальня',
				key_en: 'Bedroom',
				key_uz: 'Yotoqxona',
				val: '10,93'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,86'
			},
			{
				key_ru: 'Прач',
				key_en: 'Laundry',
				key_uz: 'Kir yuvish xonasi',
				val: '0,76'
			},
			{
				key_ru: 'Балкон',
				key_en: 'Balcony',
				key_uz: 'Balkon',
				val: '4,42'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '40',
		blockId: 'E1',
		floorId: '6',
		img: apartmentImg40,
		details: [
			{
				key_ru: 'Жилая площадь',
				key_en: 'Living room',
				key_uz: 'Mehmonxona',
				val: '19,21'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,61'
			},
			{
				key_ru: 'Прач',
				key_en: 'Laundry',
				key_uz: 'Kir yuvish xonasi',
				val: '0,95'
			},
			{
				key_ru: 'Балкон',
				key_en: 'Balcony',
				key_uz: 'Balkon',
				val: '3,92'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '41',
		blockId: 'E1',
		floorId: '6',
		img: apartmentImg41,
		details: [
			{
				key_ru: 'Жилая площадь',
				key_en: 'Living room',
				key_uz: 'Mehmonxona',
				val: '19,78'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,56'
			},
			{
				key_ru: 'Прач',
				key_en: 'Laundry',
				key_uz: 'Kir yuvish xonasi',
				val: '0,86'
			},
			{
				key_ru: 'Балкон',
				key_en: 'Balcony',
				key_uz: 'Balkon',
				val: '3,48'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '42',
		blockId: 'E1',
		floorId: '6',
		img: apartmentImg42,
		details: [
			{
				key_ru: 'Жилая площадь',
				key_en: 'Living room',
				key_uz: 'Mehmonxona',
				val: '18,19'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,46'
			},
			{
				key_ru: 'Прач',
				key_en: 'Laundry',
				key_uz: 'Kir yuvish xonasi',
				val: '0,86'
			},
			{
				key_ru: 'Балкон',
				key_en: 'Balcony',
				key_uz: 'Balkon',
				val: '3,48'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '43',
		blockId: 'E1',
		floorId: '6',
		img: apartmentImg43,
		details: [
			{
				key_ru: 'Жилая площадь',
				key_en: 'Living room',
				key_uz: 'Mehmonxona',
				val: '19,98'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,83'
			},
			{
				key_ru: 'Балкон',
				key_en: 'Balcony',
				key_uz: 'Balkon',
				val: '3,71'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '44',
		blockId: 'E1',
		floorId: '6',
		img: apartmentImg44,
		details: [
			{
				key_ru: 'Холл',
				key_en: 'Hall',
				key_uz: 'Dahliz',
				val: '2,17'
			},
			{
				key_ru: 'Кухня + Зал',
				key_en: 'Kitchen + Living room',
				key_uz: 'Oshxona + Mehmonxona',
				val: '22,31'
			},
			{
				key_ru: 'Спальня',
				key_en: 'Bedroom',
				key_uz: 'Yotoqxona',
				val: '10,67'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,72'
			},
			{
				key_ru: 'Прач.',
				key_en: 'Loundry',
				key_uz: 'Kir yuvish xonasi',
				val: '0,87'
			},
			{
				key_ru: 'Балкон',
				key_en: 'Balcony',
				key_uz: 'Balkon',
				val: '9,86'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '45',
		blockId: 'E1',
		floorId: '6',
		img: apartmentImg45,
		details: [
			{
				key_ru: 'Холл',
				key_en: 'Hall',
				key_uz: 'Dahliz',
				val: '3,22'
			},
			{
				key_ru: 'Кухня + Зал',
				key_en: 'Kitchen + Living room',
				key_uz: 'Oshxona + Mehmonxona',
				val: '26,01'
			},
			{
				key_ru: 'Спальня',
				key_en: 'Bedroom',
				key_uz: 'Yotoqxona',
				val: '16,67'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '6,72'
			},
			{
				key_ru: 'Прач.',
				key_en: 'Loundry',
				key_uz: 'Kir yuvish xonasi',
				val: '0,54'
			}
		]
	},

	// 7 - Floor
	{
		phaseId: '1',
		apartmentId: '46',
		blockId: 'E1',
		floorId: '7',
		img: apartmentImg46,
		details: [
			{
				key_ru: 'Холл',
				key_en: 'Hall',
				key_uz: 'Dahliz',
				val: '7,74'
			},
			{
				key_ru: 'Кухня + Зал',
				key_en: 'Kitchen + Living room',
				key_uz: 'Oshxona + Mehmonxona',
				val: '35,29'
			},
			{
				key_ru: 'Спальня',
				key_en: 'Bedroom',
				key_uz: 'Yotoqxona',
				val: '12,06'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '5,09'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '4,95'
			},
			{
				key_ru: 'Прач',
				key_en: 'Laundry',
				key_uz: 'Kir yuvish xonasi',
				val: '0,90'
			},
			{
				key_ru: 'Детская',
				key_en: 'Childrens room',
				key_uz: 'Bolalar xonasi',
				val: '9,60'
			},
			{
				key_ru: 'Детская',
				key_en: 'Childrens room',
				key_uz: 'Bolalar xonasi',
				val: '10,13'
			},
			{
				key_ru: 'Терасса',
				key_en: 'Terrace',
				key_uz: 'Terassa',
				val: '11,33'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '47',
		blockId: 'E1',
		floorId: '7',
		img: apartmentImg47,
		details: [
			{
				key_ru: 'Холл',
				key_en: 'Hall',
				key_uz: 'Dahliz',
				val: '13,60'
			},
			{
				key_ru: 'Кухня + Зал',
				key_en: 'Kitchen + Living room',
				key_uz: 'Oshxona + Mehmonxona',
				val: '35,31'
			},
			{
				key_ru: 'Спальня',
				key_en: 'Bedroom',
				key_uz: 'Yotoqxona',
				val: '15,97'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,83'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,71'
			},
			{
				key_ru: 'Прач',
				key_en: 'Laundry',
				key_uz: 'Kir yuvish xonasi',
				val: '1,04'
			},
			{
				key_ru: 'Детская',
				key_en: 'Childrens room',
				key_uz: 'Bolalar xonasi',
				val: '11,43'
			},
			{
				key_ru: 'Детская',
				key_en: 'Childrens room',
				key_uz: 'Bolalar xonasi',
				val: '10,95'
			},
			{
				key_ru: 'Терасса',
				key_en: 'Terrace',
				key_uz: 'Terassa',
				val: '30,79'
			}
		]
	},
	{
		phaseId: '1',
		apartmentId: '48',
		blockId: 'E1',
		floorId: '7',
		img: apartmentImg48,
		details: [
			{
				key_ru: 'Холл',
				key_en: 'Hall',
				key_uz: 'Dahliz',
				val: '9,73'
			},
			{
				key_ru: 'Кухня + Зал',
				key_en: 'Kitchen + Living room',
				key_uz: 'Oshxona + Mehmonxona',
				val: '35,71'
			},
			{
				key_ru: 'Спальня',
				key_en: 'Bedroom',
				key_uz: 'Yotoqxona',
				val: '17,32'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '3,85'
			},
			{
				key_ru: 'C/У',
				key_en: 'Bathroom',
				key_uz: 'Hammom',
				val: '5,72'
			},
			{
				key_ru: 'Прач',
				key_en: 'Laundry',
				key_uz: 'Kir yuvish xonasi',
				val: '1,96'
			},
			{
				key_ru: 'Детская',
				key_en: 'Childrens room',
				key_uz: 'Bolalar xonasi',
				val: '15,36'
			},
			{
				key_ru: 'Детская',
				key_en: 'Childrens room',
				key_uz: 'Bolalar xonasi',
				val: '23,15'
			},
			{
				key_ru: 'Терасса',
				key_en: 'Terrace',
				key_uz: 'Terassa',
				val: '23,31'
			},
			{
				key_ru: 'Терасса',
				key_en: 'Terrace',
				key_uz: 'Terassa',
				val: '8,85'
			}
		]
	}
];

// apartments sketch paths (SVG) (E1, 7 floors)
const apartmentsPaths1 = [
	{
		path: 'M 4.124 243.794 L 5.37 540.289 L 311.831 539.043 L 308.094 245.04 L 4.124 243.794 Z',
		apartmentId: '2'
	},
	{
		path: 'M 299.995 192.716 L 301.478 534.616 L 666.252 533.18 L 661.804 194.153 L 299.995 192.716 Z',
		apartmentId: '3'
	},
	{
		path: 'M 643.61 190.67 L 645.023 541.726 L 992.417 540.252 L 988.187 192.145 L 643.61 190.67 Z',
		apartmentId: '4'
	},
	{
		path: 'M 15.336 572.947 L 102.54 577.93 L 103.786 610.32 L 151.125 610.32 L 152.371 651.431 L 213.414 646.448 L 214.66 612.812 L 486.24 611.566 L 486.24 901.832 L 12.844 899.341',
		apartmentId: '1'
	}
];
const apartmentsPaths2 = [
	{
		path: 'M 815.281 201.927 L 815.281 173.183 L 848.875 173.183 L 848.875 146.271 L 994.815 146.271 L 994.815 205.706 L 994.815 730.146 L 842.447 730.146 L 840.78 577.274 L 840.78 526.524 L 800.276 526.524 L 800.276 484.071 L 820.12 484.071 L 820.12 206.524 L 815.281 201.927 Z',
		apartmentId: '11'
	},
	{
		path: 'M 653.426 532.074 L 653.426 199.678 L 647.472 194.482 L 647.472 141.865 L 802.707 141.865 L 802.707 166.671 L 828.151 166.671 L 814.178 198.826 L 814.178 494.113 L 804.257 493.342 L 804.257 528.501 L 747.178 528.501 L 747.178 532.074 L 658.189 532.074 L 653.426 532.074 Z',
		apartmentId: '10'
	},
	{
		path: 'M 657.303 524.52 L 657.303 199.583 L 654.93 195.337 L 654.93 139.929 L 503.063 139.929 L 503.063 168.149 L 481.197 168.149 L 481.197 198.829 L 475.9 487.492 L 466.366 487.492 L 466.366 518.408 L 562.343 518.408 L 562.343 525.355 L 657.303 525.355 L 657.303 524.52 Z',
		apartmentId: '9'
	},
	{
		path: 'M 303.725 524.768 L 303.725 199.34 L 292.48 195.085 L 292.48 139.594 L 455.35 139.594 L 455.35 167.858 L 482.044 167.858 L 482.044 198.582 L 482.044 487.682 L 467.385 487.682 L 467.385 518.646 L 397.09 518.646 L 397.09 525.602 L 303.725 525.602 L 303.725 524.768 Z',
		apartmentId: '8'
	},
	{
		path: 'M 7.442 187.014 L 7.442 187.884 L 7.442 321.804 L 11.537 321.804 L 11.537 415.641 L 11.537 523.156 L 160.093 523.156 L 229.243 523.156 L 311.527 523.156 L 311.527 247.887 L 204.033 247.887 L 204.033 187.014 L 7.442 187.014 Z',
		apartmentId: '7'
	},
	{
		path: 'M 10.515 563.578 L 8.264 878.8 L 4.093 883.411 L 3.683 940.673 L 200.434 940.673 L 200.876 878.8 L 303.717 878.8 L 311.081 779.499 L 312.404 594.127 L 222.713 594.127 L 222.444 631.967 L 153.167 631.967 L 153.432 594.861 C 153.435 594.371 100.022 594.861 100.022 594.861 L 100.246 563.578 L 10.515 563.578 Z',
		apartmentId: '6'
	},
	{
		path: 'M 308.283 594.37 L 308.283 876.773 L 306.124 876.773 L 306.124 937.971 L 490.901 937.971 L 490.901 876.692 L 646.703 876.692 L 643.221 834.593 L 643.221 593.64 L 341.148 593.64 L 308.283 594.37 Z',
		apartmentId: '5'
	}
];
const apartmentsPaths3 = [
	{
		path: 'M 815.281 201.927 L 815.281 173.183 L 848.875 173.183 L 848.875 146.271 L 994.815 146.271 L 994.815 205.706 L 994.815 730.146 L 842.447 730.146 L 840.78 577.274 L 840.78 526.524 L 800.276 526.524 L 800.276 484.071 L 820.12 484.071 L 820.12 206.524 L 815.281 201.927 Z',
		apartmentId: '18'
	},
	{
		path: 'M 653.426 532.074 L 653.426 199.678 L 647.472 194.482 L 647.472 141.865 L 802.707 141.865 L 802.707 166.671 L 828.151 166.671 L 814.178 198.826 L 814.178 494.113 L 804.257 493.342 L 804.257 528.501 L 747.178 528.501 L 747.178 532.074 L 658.189 532.074 L 653.426 532.074 Z',
		apartmentId: '17'
	},
	{
		path: 'M 657.303 524.52 L 657.303 199.583 L 654.93 195.337 L 654.93 139.929 L 503.063 139.929 L 503.063 168.149 L 481.197 168.149 L 481.197 198.829 L 475.9 487.492 L 466.366 487.492 L 466.366 518.408 L 562.343 518.408 L 562.343 525.355 L 657.303 525.355 L 657.303 524.52 Z',
		apartmentId: '16'
	},
	{
		path: 'M 303.725 524.768 L 303.725 199.34 L 292.48 195.085 L 292.48 139.594 L 455.35 139.594 L 455.35 167.858 L 482.044 167.858 L 482.044 198.582 L 482.044 487.682 L 467.385 487.682 L 467.385 518.646 L 397.09 518.646 L 397.09 525.602 L 303.725 525.602 L 303.725 524.768 Z',
		apartmentId: '15'
	},
	{
		path: 'M 7.442 187.014 L 7.442 187.884 L 7.442 321.804 L 11.537 321.804 L 11.537 415.641 L 11.537 523.156 L 160.093 523.156 L 229.243 523.156 L 311.527 523.156 L 311.527 247.887 L 204.033 247.887 L 204.033 187.014 L 7.442 187.014 Z',
		apartmentId: '14'
	},
	{
		path: 'M 10.515 563.578 L 8.264 878.8 L 4.093 883.411 L 3.683 940.673 L 200.434 940.673 L 200.876 878.8 L 303.717 878.8 L 311.081 779.499 L 312.404 594.127 L 222.713 594.127 L 222.444 631.967 L 153.167 631.967 L 153.432 594.861 C 153.435 594.371 100.022 594.861 100.022 594.861 L 100.246 563.578 L 10.515 563.578 Z',
		apartmentId: '13'
	},
	{
		path: 'M 308.283 594.37 L 308.283 876.773 L 306.124 876.773 L 306.124 937.971 L 490.901 937.971 L 490.901 876.692 L 646.703 876.692 L 643.221 834.593 L 643.221 593.64 L 341.148 593.64 L 308.283 594.37 Z',
		apartmentId: '12'
	}
];
const apartmentsPaths4 = [
	{
		path: 'M 957.982 123.5 L 957.982 87.549 L 996.71 87.549 L 996.71 53.888 L 1164.97 53.888 L 1164.97 128.225 L 1163.05 529.478 L 987.377 529.478 L 940.682 529.478 L 940.682 476.38 L 963.562 476.38 L 963.562 129.248 L 957.982 123.5 Z',
		apartmentId: '25'
	},
	{
		path: 'M 766.65 528.991 L 766.65 130.222 L 759.617 123.987 L 759.617 60.853 L 942.98 60.853 L 942.98 90.618 L 973.032 90.618 L 956.529 129.199 L 956.529 483.492 L 944.808 482.567 L 944.808 524.752 L 877.39 524.752 L 877.39 528.991 L 772.276 528.991 L 766.65 528.991 Z',
		apartmentId: '24'
	},
	{
		path: 'M 759.899 528.016 L 759.899 129.199 L 757.274 123.987 L 757.274 55.983 L 589.242 55.983 L 589.242 90.618 L 565.05 90.618 L 565.05 128.225 L 559.191 482.567 L 548.641 482.567 L 548.641 520.514 L 654.832 520.514 L 654.832 528.991 L 759.899 528.991 L 759.899 528.016 Z',
		apartmentId: '23'
	},
	{
		path: 'M 364.389 525.142 L 364.389 126.325 L 351.731 121.113 L 351.731 53.109 L 535.092 53.109 L 535.092 87.744 L 565.145 87.744 L 565.145 125.35 L 565.145 479.693 L 548.641 479.693 L 548.641 517.64 L 469.502 517.64 L 469.502 526.117 L 364.389 526.117 L 364.389 525.142 Z',
		apartmentId: '22'
	},
	{
		path: 'M 4.792 109.567 L 4.792 110.591 L 4.792 275.68 L 9.527 275.68 L 9.527 391.326 L 9.527 523.827 L 181.356 523.827 L 261.339 523.827 L 356.513 523.827 L 356.513 184.586 L 232.177 184.586 L 232.177 109.567 L 4.792 109.567 Z',
		apartmentId: '21'
	},
	{
		path: 'M 18.951 577.217 L 18.951 954.21 L 14.169 959.715 L 14.169 1028.17 L 241.46 1028.17 L 241.46 954.21 L 360.263 954.21 L 367.952 835.447 L 367.952 613.751 L 264.34 613.751 L 264.34 659.006 L 184.309 659.006 L 184.309 614.628 C 184.309 614.044 122.61 614.628 122.61 614.628 L 122.61 577.217 L 18.951 577.217 Z',
		apartmentId: '20'
	},
	{
		path: 'M 377.001 608.442 L 361.201 956.207 L 356.419 961.712 L 356.419 1030.17 L 583.711 1030.17 L 583.711 956.207 L 757.274 956.207 L 757.274 608.442 L 520.511 608.442 L 377.001 608.442 Z',
		apartmentId: '19'
	},
	{
		path: 'M 1168.53 184.635 C 1167.74 283.669 1166.66 483.248 1168.53 489.094 L 1236.52 489.094 L 1236.52 574.343 L 1557.67 574.343 L 1557.67 559.728 L 1721.76 559.728 C 1720.21 505.315 1721.76 408.717 1721.76 379.489 C 1721.76 364.826 1721.76 379.489 1637.37 379.489 L 1637.37 204.12 L 1482.66 204.12 L 1482.66 184.635 L 1168.53 184.635 Z',
		apartmentId: '26'
	},
	{
		path: 'M 998.72 615.75 L 998.72 786.246 L 1162.81 786.246 L 1162.81 961.615 L 1561.33 961.615 L 1561.33 937.258 L 1716.04 937.258 L 1716.04 581.65 L 1561.33 581.65 L 1242.52 581.65 L 1242.52 615.75 L 998.72 615.75 Z',
		apartmentId: '27'
	}
];
const apartmentsPaths5 = [
	{
		path: 'M 1057.95 121.504 L 1057.95 85.515 L 1100.71 85.515 L 1100.71 51.867 L 1286.46 51.867 L 1286.46 126.233 L 1284.34 527.917 L 1090.41 527.917 L 1038.86 527.917 L 1038.86 474.761 L 1064.12 474.761 L 1064.12 127.307 L 1057.95 121.504 Z',
		apartmentId: '34'
	},
	{
		path: 'M 846.734 527.476 L 846.734 128.233 L 838.974 121.99 L 838.974 58.792 L 1041.4 58.792 L 1041.4 88.635 L 1074.57 88.635 L 1056.35 127.209 L 1056.35 481.88 L 1043.41 480.955 L 1043.41 523.185 L 968.989 523.185 L 968.989 527.476 L 853 527.476 L 846.734 527.476 Z',
		apartmentId: '33'
	},
	{
		path: 'M 839.281 526.453 L 839.281 127.209 L 836.385 121.99 L 836.385 53.914 L 650.84 53.914 L 650.84 88.635 L 624.187 88.635 L 624.187 126.282 L 617.716 480.955 L 606.07 480.955 L 606.07 518.943 L 723.297 518.943 L 723.297 527.476 L 839.281 527.476 L 839.281 526.453 Z',
		apartmentId: '32'
	},
	{
		path: 'M 402.72 523.576 L 402.72 124.332 L 388.696 119.113 L 388.696 51.036 L 591.112 51.036 L 591.112 85.709 L 624.289 85.709 L 624.289 123.404 L 624.289 478.077 L 606.07 478.077 L 606.07 516.066 L 518.706 516.066 L 518.706 524.601 L 402.72 524.601 L 402.72 523.576 Z',
		apartmentId: '31'
	},
	{
		path: 'M 5.7 107.557 L 5.7 108.628 L 5.7 273.846 L 10.927 273.846 L 10.927 389.617 L 10.927 522.259 L 200.613 522.259 L 288.909 522.259 L 393.973 522.259 L 393.973 182.656 L 256.716 182.656 L 256.716 107.557 L 5.7 107.557 Z',
		apartmentId: '30'
	},
	{
		path: 'M 21.331 575.705 L 21.331 953.104 L 16.052 958.613 L 16.052 1027.18 L 266.964 1027.18 L 266.964 953.104 L 398.115 953.104 L 406.604 834.212 L 406.604 612.278 L 292.222 612.278 L 292.222 657.583 L 203.873 657.583 L 203.873 613.157 C 203.873 612.572 135.763 613.157 135.763 613.157 L 135.763 575.705 L 21.331 575.705 Z',
		apartmentId: '29'
	},
	{
		path: 'M 416.591 606.966 L 399.15 955.1 L 393.871 960.612 L 393.871 1029.18 L 644.783 1029.18 L 644.783 955.1 L 836.385 955.1 L 836.385 606.966 L 575.016 606.966 L 416.591 606.966 Z',
		apartmentId: '28'
	},
	{
		path: 'M 1290.04 182.703 C 1289.46 281.845 1288.17 471.883 1289.62 477.736 L 1356.12 477.736 L 1356.54 558.199 L 1591.66 558.199 L 1718.83 558.199 C 1717.64 503.728 1718.83 407.027 1718.83 377.767 C 1718.83 363.088 1698.86 377.767 1633.44 377.767 L 1633.44 182.703 L 1290.04 182.703 Z',
		apartmentId: '35'
	},
	{
		path: 'M 1102.93 614.279 L 1102.93 784.96 L 1284.08 784.96 L 1284.08 960.514 L 1724.01 960.514 L 1724.01 936.132 L 1724.01 570.39 L 1372.06 570.39 L 1372.06 614.279 L 1102.93 614.279 Z',
		apartmentId: '36'
	}
];
const apartmentsPaths6 = [
	{
		path: 'M 1055.77 112.494 L 1055.77 76.102 L 1098.67 76.102 L 1098.67 42.033 L 1285.02 42.033 L 1285.02 117.267 L 1282.9 523.492 L 1088.31 523.492 L 1036.6 523.492 L 1036.6 469.738 L 1061.93 469.738 L 1061.93 118.327 L 1055.77 112.494 Z',
		apartmentId: '43'
	},
	{
		path: 'M 843.855 523.04 L 843.855 119.294 L 836.05 112.971 L 836.05 49.055 L 1039.14 49.055 L 1039.14 79.215 L 1072.42 79.215 L 1054.14 118.263 L 1054.14 476.963 L 1041.15 476.002 L 1041.15 518.73 L 966.49 518.73 L 966.49 523.04 L 850.137 523.04 L 843.855 523.04 Z',
		apartmentId: '42'
	},
	{
		path: 'M 836.36 522.013 L 836.36 118.263 L 833.445 112.971 L 833.445 44.124 L 647.344 44.124 L 647.344 79.215 L 620.551 79.215 L 620.551 117.301 L 614.063 476.002 L 602.389 476.002 L 602.389 514.415 L 720.019 514.415 L 720.019 523.04 L 836.36 523.04 L 836.36 522.013 Z',
		apartmentId: '41'
	},
	{
		path: 'M 398.37 519.105 L 398.37 115.353 L 384.284 110.062 L 384.284 41.215 L 587.374 41.215 L 587.374 76.305 L 620.68 76.305 L 620.68 114.39 L 620.68 473.095 L 602.389 473.095 L 602.389 511.509 L 514.737 511.509 L 514.737 520.131 L 398.37 520.131 L 398.37 519.105 Z',
		apartmentId: '40'
	},
	{
		path: 'M 0.008 98.375 L 0.008 99.444 L 0.008 266.548 L 5.271 266.548 L 5.271 383.652 L 5.271 517.774 L 195.603 517.774 L 284.197 517.774 L 389.599 517.774 L 389.599 174.331 L 251.871 174.331 L 251.871 98.375 L 0.008 98.375 Z',
		apartmentId: '39'
	},
	{
		path: 'M 15.72 571.825 L 15.72 953.49 L 10.392 959.078 L 10.392 1028.39 L 262.152 1028.39 L 262.152 953.49 L 393.739 953.49 L 402.253 833.254 L 402.253 608.834 L 287.486 608.834 L 287.486 654.645 L 198.854 654.645 L 198.854 609.701 C 198.854 609.127 130.525 609.701 130.525 609.701 L 130.525 571.825 L 15.72 571.825 Z',
		apartmentId: '38'
	},
	{
		path: 'M 412.276 603.437 L 394.784 955.514 L 389.483 961.105 L 389.483 1030.42 L 641.216 1030.42 L 641.216 955.514 L 833.445 955.514 L 833.445 603.437 L 571.237 603.437 L 412.276 603.437 Z',
		apartmentId: '37'
	},
	{
		path: 'M 1288.67 174.373 C 1288.02 274.649 1286.73 466.829 1288.19 472.746 L 1354.92 472.746 L 1355.32 554.119 L 1591.21 554.119 L 1718.82 554.119 C 1717.63 499.046 1718.82 127.521 1718.82 97.928 L 1422.84 102.86 L 1422.84 174.373 L 1288.67 174.373 Z',
		apartmentId: '44'
	},
	{
		path: 'M 1100.88 614.917 L 1100.88 785.851 L 1282.64 785.851 L 1282.64 961.669 L 1724.01 961.669 L 1724.01 937.251 L 1724.01 570.963 L 1370.88 570.963 L 1370.88 614.917 L 1100.88 614.917 Z',
		apartmentId: '45'
	}
];
const apartmentsPaths7 = [
	{
		path: 'M 5.148 118.936 L 5.148 120.014 L 5.148 287.364 L 10.343 287.364 L 10.343 404.65 L 10.343 538.966 L 198.948 538.966 L 286.741 538.966 L 1036.94 538.966 L 1036.94 61.691 L 383.385 61.691 L 383.385 118.936 L 254.734 118.936 L 5.148 118.936 Z',
		apartmentId: '47'
	},
	{
		path: 'M 836.244 987.79 L 836.244 629.7 L 355.082 629.7 L 355.082 550.672 L 10.293 550.672 L 0 1057.58 L 625.252 1057.58 L 625.252 987.79 L 836.244 987.79 Z',
		apartmentId: '46'
	},
	{
		path: 'M 1042.08 61.691 L 1042.08 535.854 L 1090.97 535.854 L 1096.12 814.918 L 1283.96 814.918 L 1283.96 985.321 L 1723.95 985.321 L 1723.95 123.432 L 1430.61 123.432 L 1430.61 190.111 L 1286.53 190.111 L 1286.53 61.691 L 1042.08 61.691 Z',
		apartmentId: '48'
	}
];
export const apartmentsPaths = [
	{
		floorId: '1',
		paths: apartmentsPaths1
	},
	{
		floorId: '2',
		paths: apartmentsPaths2
	},
	{
		floorId: '3',
		paths: apartmentsPaths3
	},
	{
		floorId: '4',
		paths: apartmentsPaths4
	},
	{
		floorId: '5',
		paths: apartmentsPaths5
	},
	{
		floorId: '6',
		paths: apartmentsPaths6
	},
	{
		floorId: '7',
		paths: apartmentsPaths7
	}
];

// {floor_id} apartments sketches (E1)
export const apartmentsSketches = [
	{
		phaseId: '1',
		blockId: 'E1',
		floorId: '1',
		img: apartmentsSketchImg1,
		paths: apartmentsPaths.find(p => p.floorId == '1'),
		apartments: apartments.filter(a => a.blockId == 'E1' && a.floorId == '1')
	},
	{
		phaseId: '1',
		blockId: 'E1',
		floorId: '2',
		img: apartmentsSketchImg2,
		paths: apartmentsPaths.find(p => p.floorId == '2'),
		apartments: apartments.filter(a => a.blockId == 'E1' && a.floorId == '2')
	},
	{
		phaseId: '1',
		blockId: 'E1',
		floorId: '3',
		img: apartmentsSketchImg3,
		paths: apartmentsPaths.find(p => p.floorId == '3'),
		apartments: apartments.filter(a => a.blockId == 'E1' && a.floorId == '3')
	},
	{
		phaseId: '1',
		blockId: 'E1',
		floorId: '4',
		img: apartmentsSketchImg4,
		paths: apartmentsPaths.find(p => p.floorId == '4'),
		apartments: apartments.filter(a => a.blockId == 'E1' && a.floorId == '4')
	},
	{
		phaseId: '1',
		blockId: 'E1',
		floorId: '5',
		img: apartmentsSketchImg5,
		paths: apartmentsPaths.find(p => p.floorId == '5'),
		apartments: apartments.filter(a => a.blockId == 'E1' && a.floorId == '5')
	},
	{
		phaseId: '1',
		blockId: 'E1',
		floorId: '6',
		img: apartmentsSketchImg6,
		paths: apartmentsPaths.find(p => p.floorId == '6'),
		apartments: apartments.filter(a => a.blockId == 'E1' && a.floorId == '6')
	},
	{
		phaseId: '1',
		blockId: 'E1',
		floorId: '7',
		img: apartmentsSketchImg7,
		paths: apartmentsPaths.find(p => p.floorId == '7'),
		apartments: apartments.filter(a => a.blockId == 'E1' && a.floorId == '7')
	}
];
